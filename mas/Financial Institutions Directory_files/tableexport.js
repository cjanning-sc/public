...
;(function (root, factory) {
    if (typeof define === 'function' && define.amd) {
        ...
        var TableExport = function (selectors, options, isUpdate) {
            ...
            self.selectors.each(function () {
                ...
                function createObjButton(dataObject, myContent, myClass) {
                    ...
                    var exportButton = "<button data-fileblob='" + dataObject + "' class='" + bootstrapClass + bootstrapTheme + myClass + " export-table'>" + myContent + "</button>";
                    checkCaption(exportButton);
                }
            });
            ...
            TableExport.prototype.assignFileSize("button[data-fileblob]", "button[data-fileblob]");
            return self;
        };
        ...
        TableExport.prototype = {
            ...
            /**
             * Get the generated file size, this is custom (not from tableexport.js)
             * @memberof TableExport.prototype
             * @param data {String}
             * @param mime {String} mime type
             * @param name {String} filename
             * @param extension {String} file extension
             */
            getFileSize: function (data, mime, name, extension) {
                if (XLSX && extension.substr(0, 4) == (".xls")) {
                    var wb = new this.Workbook(),
                        ws = this.createSheet(data);
            ...
            /**
             * Assign File Size to Button
             * @memberof TableExport.prototype
             * @param buttonId {String} Button that will be assigned
             * @param excelId {String} Excel Object with File Blob
             */
            assignFileSize: function (buttonId, excelId) {
                var btn = $(buttonId);
                var xl = $(excelId);                
                var object = xl.data("fileblob"),
                        data = object.data,
                        fileName = object.fileName,
                        mimeType = object.mimeType,
                        fileExtension = object.fileExtension;
                var size = TableExport.prototype.getFileSize(data, mimeType, fileName, fileExtension);
                btn.text("Export to xls ("+size+" KB)");
            },
